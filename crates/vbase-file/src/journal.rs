...
impl File {
    ...
    fn read_fragment(&mut self) -> Result<Option<FragmentKind>> {
        ...
        if self.length - self.offset < HEADER_SIZE {
            return self.name.corrupted("incomplete fragment");
        }
        ...
        if dec.len() < size {
            return self.name.corrupted(format!(
...
impl FileWriter {
    ...
    pub fn sync(&mut self) -> Result<()> {
        self.file.sync().context(|| format!("sync {}", self.name))
    }
...
impl FileWriter {
    fn flush(&mut self) -> Result<()> {
        ...
        let buf = &self.buffer[self.offset..self.length];
        self.file
            .write_exact(buf)
            .context(|| format!("write {}", self.name))?;
    ...
    fn test_file() -> Result<()> {
        ...
        ];
        {
            let mut file = dir
                ...
                .map(|file| FileWriter::open(file, name.into()))?;
        ...
        }
        {
            let mut file = dir
                ...
                .map(|file| File::open(file, name.into()))?;
            for record in &records {
