    ...
    ($t:ty) => {
        ...
        impl<'de> Decode<'de> for $t {
            fn decode_from<D: Decoder<'de>>(dec: &mut D) -> Self {
                ...
                <$t>::from_le_bytes(unsafe { *(bytes.as_ptr() as *const _) })
            }
        }
...
fn take_mut(buf: &mut &mut [u8], len: usize) -> *mut u8 {
    assert!(buf.len() >= len);
...
impl Encoder for &mut [u8] {
    ...
    fn append(&mut self, bytes: &[u8]) {
        let ptr = take_mut(self, bytes.len());
        unsafe {
...
pub fn test_codec<'de, T>(value: T)
where
    T: Encode + Decode<'de> + Copy + PartialEq + std::fmt::Debug,
{
    let mut enc = Vec::new();
    enc.encode(value);
